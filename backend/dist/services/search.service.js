...
class SearchService {
    async semanticSearch(query, page = 1, limit = 20) {
        const publishedJobs = await database_1.prisma.job.findMany({
            where: { status: client_1.JobStatus.PUBLISHED },
        ...
        try {
            ...
            const queryEmbedding = await ai_provider_1.aiProvider.generateEmbedding(query);
